# 🎯 Wedding Project Requirements (v1.0)

## 1. 프로젝트 목표 (Project Goal)
소중한 분들을 초대하기 위한 **"세상에 단 하나뿐인 모바일 청첩장"**. 단순히 정보 전달을 넘어 신랑 신부의 이야기를 담고, 하객들과 소통할 수 있는 우아한 디지털 공간을 구축함.

## 2. 디자인 컨셉 (Design Concept)
- **Reference**: [Coastal World](https://coastalworld.com/)와 거의 동일한 조작감, 색감, 분위기를 지향함.
- **Modern & Elegant + Playful**: 화이트/아이보리 베이스에 Coastal World 스타일의 밝고 선명한 포인트 컬러 활용.
- **High-End Web3D**: 저사양 기기에서도 부드럽게 구동되는 최적화된 Low-poly 스타일.

## 3. 기능 요구사항 (Functional Requirements)

### 3.1 메인 페이지 (Main/Intro) - 3D 여정
- **시나리오 (Stage 1)**: **1호선 독산역**을 모티브로 한 3D 공간.
- **조작 (UX)**: Coastal World 스타일의 **부드러운 캐릭터 이동 및 카메라 추적**. 모바일 터치 조작 최적화.
- **타임라인 UI**: 화면 상단 중앙에 **2018-02-15부터 현재까지 흐르는 시간**을 실시간 노출.
- **캐릭터**: Three.js 기반의 **3D 신랑/신부 모델** (Coastal World 수준의 자연스러운 애니메이션).
- **인터랙션**: 이동 중 특정 지점(오브젝트) 접근 시 Coastal World처럼 자연스럽게 정보 팝업 노출.

### 3.2 상세 정보 (Wedding Details)
- **일시 및 장소**: 정확한 예식 날짜, 시간, 예식장 이름 및 홀 정보.
- **지도 연동**: 카카오맵/네이버지도/구글지도 API 연동 및 '길 찾기' 버튼.
- **교통 안내**: 지하철, 버스, 자차 이용 시 상세 안내 (주차 정보 포함).

### 3.3 갤러리 (Gallery)
- **이미지 뷰어**: 탭 또는 스와이프 방식의 사진첩.
- **풀스크린 모드**: 사진 터치 시 크게 보기 지원.

### 3.4 RSVP & 소통 (RSVP & Communication)
- **참석 여부 확인**: 하객 이름, 동반 인원, 식사 여부를 입력받아 전송 (Admin 페이지 또는 이메일 알림).
- **방명록**: 축하 메시지를 남길 수 있는 실시간 댓글 시스템.

### 3.5 부가 기능 (Utilities)
- **축의금 전달**: 신랑/신부/혼주 계좌 정보 제공 및 카카오페이/토스 링크.
- **공유하기**: 카카오톡 공유 API를 통한 청첩장 전달 기능.
- **음악**: 배경 음악 재생/정지 기능 (선택 사항).

## 4. 기술 및 성능 요구사항 (Technical & Performance)

### 4.1 런타임 성능 (Runtime Performance)
- **Target FPS**: 모든 타겟 기기(모바일 WebView 포함)에서 상시 **60 FPS** 유지.
- **No Stuttering**: 횡스크롤 여정 중 프레임 드랍(Stuttering) 및 렉 현상 제로.
- **Input Latency**: 사용자 입력(Touch/Keyboard)에 대한 즉각적인 반응 (Latency < 16ms).

### 4.2 리소스 관리 (Resource Optimization)
- **Asset Size**: 개별 GLB 파일은 **500KB 미만**으로 최적화하며, 텍스처는 WebP 포맷을 사용하여 총 에셋 크기 최소화.
- **Draw Calls**: 씬 내 전체 드로우 콜을 **50개 이하**로 제어 (Instancing 적극 활용).
- **Memory Hygiene**: 페이지 전환 또는 컴포넌트 언마운트 시 모든 Three.js 객체(Geometry, Material, Texture)를 명시적으로 `dispose()` 하여 메모리 누수 방지.

### 4.3 플랫폼 최적화 (Platform Optimization)
- **WebView Performance**: 안드로이드/iOS WebView 환경에서의 GPU 가속 및 하드웨어 성능을 최대한 활용할 수 있도록 쉐이더(Shader) 복잡도 관리.
- **Mobile Responsive**: 다양한 해상도(Aspect Ratio)에서 카메라 시야각(FOV)이 깨지지 않는 반응형 3D 카메라 시스템 구축.

## 5. 기술 스택 (Technical Stack)
- **Engine**: Three.js (ES Modules 기반 완전 모듈화)
- **Physics**: 자체 간소화된 물리 엔진 (중력, 점프, 충돌)
- **Assets**: GLB (Low-Poly + Baked Lightmap)
- **Build**: Vite/Webpack (Tree-shaking 필수 적용)

---
*Last Updated: 2026-03-13*
